...
# Throttled concurrency to respect Free Tier 15 RPM
MAX_WORKERS = 3 
...
def extract_text_from_pdf(file_bytes: bytes):
    ...
    for page_num in range(len(pdf_document)):
        ...
        for line in page_text.split("\n"):
            line_str = line.strip()
            if bool(re.search(r"[\u0600-\u06FFa-zA-Z]", line_str)):
...
def extract_text_from_docx(file_bytes: bytes):
    ...
    for p in doc.element.body.iter():
        if p.tag.endswith("}p"):
            ...
            if bool(re.search(r"[\u0600-\u06FFa-zA-Z]", clean_text)):
...
def run_parallel_translations(paras, glossary_data, progress_bar):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {
            executor.submit(translate_with_ai, text, glossary_data): (i, text) 
